...
def Ma(A, B, C):
    ...
    c1=C[2:]
    for i in range(32):
        sum=int(a1[i])+ int(b1[i]) + int(c1[i])
        #print(sum)
        if sum>1:
            majority=majority+'1'
        else:
...
def Sum0(A):
    ...
    arot1=rightShift(A,2)
    arot2=rightShift(A,13)
    arot3=rightShift(A,22)
    sum0=''
...
def Sum1(E):
    ...
    :return: A 32 bit binary string with the result
    """
    erot1=rightShift(E,6)
    erot2=rightShift(E,11)
    erot3=rightShift(E,25)
    sum1=''
